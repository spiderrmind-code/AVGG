...
export async function findDuplicateGroups(): Promise<DuplicateReport[]> {
  ...
  for (const candidate of uniqueIndexCandidates) {
    const groups = await db.collection(candidate.collection).aggregate([{ $match: candidate.partialFilterExpression }, { $group: { _id: `$${candidate.field}`, count: { $sum: 1 } } }, { $match: { count: { $gt: 1 } } }, { $group: { _id: null, groups: { $sum: 1 }, documents: { $sum: "$count" } } }]).toArray();
