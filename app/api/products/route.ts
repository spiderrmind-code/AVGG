...
export async function GET(request: Request) {
  try {
    ...
    const sanitized = await getPublicCatalog({ limit, page, ...(category ? { category } : {}), featured });

    return NextResponse.json({ success: true, count: sanitized.length, products: sanitized, page, limit, hasMore: sanitized.length === limit });
  } catch (error) {
